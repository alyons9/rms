...
import view.changePrice.changeDrinkPrice;
import view.removeMenuItem.removeDessertItemDialog;
import view.removeMenuItem.removeDinnerItemDialog;
import view.removeMenuItem.removeDrinkItemDialog;

/**
 *
 * @author SAMIR
 */
public class adminEditDrinkMenu extends javax.swing.JPanel implements ActionListener {
    private DrinksDao nodeList;
    private Vector <Drink> drink;
    ...
    private String list[];
    private DrinksDao secList;
    public JPanel dialog;
    /** Creates new form addAppetizerMenu */
    public adminEditDrinkMenu () throws SAXException, ParserConfigurationException {
    ...
    public void setUpComponents() throws SAXException, ParserConfigurationException{
       ...
       sizeOfList = nodeList.length();
        ...
        drink = new Vector(sizeOfList);
        System.out.println(sizeOfList);
                
        //store all the food in to the food array variable
        drink = nodeList.getVectorAllDrinks();
    ...
    System.out.println(drink.get(0).getPic());
...
//        appIconPic[i] = new ImageIcon(appIconName[i]);
       icon.add(i, createImageIcon(drink.get(i).getPic()));
        nameLabels.add(i, new JLabel(drink.get(i).getName()));//appIconPic[i],JLabel.CENTER);
        nameLabels.get(i).setIcon(icon.get(i));
        //nameLabels.get(i).setIcon(new javax.swing.ImageIcon(getClass().getResource(drink.get(i).getPic())));
        buttons.add(i, new JComboBox(list));
        buttons.get(i).setPreferredSize(new Dimension(70,30));
        buttons.get(i).setFocusCycleRoot(true);
         buttons.get(i).setActionCommand(drink.get(i).getName());
         buttons.get(i).addActionListener(this);
       
        prices.add(i,new JLabel(""+drink.get(i).getPrice()));
...
//          add(dialog);
            ...
            System.out.println("im trying to display the dialgo");
              ...
              changeDrinkPrice price = null;
            try {
                DrinksDao secList2 = new DrinksDao();
                price = new changeDrinkPrice(secList2,this, aCommand);
                repaint();
            } catch (SAXException ex) {
                Logger.getLogger(adminEditDrinkMenu.class.getName()).log(Level.SEVERE, null, ex);
            } catch (ParserConfigurationException ex) {
                Logger.getLogger(adminEditDrinkMenu.class.getName()).log(Level.SEVERE, null, ex);
            }
